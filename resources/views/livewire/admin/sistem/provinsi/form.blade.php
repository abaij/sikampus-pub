@section('title', ($provinsiId ? 'Ubah' : 'Tambah') . ' Provinsi — ' . config('app.name'))
@section('header_title', ($provinsiId ? 'Ubah' : 'Tambah') . ' Provinsi')
    ...
    <form wire:submit="save" class="space-y-6">
        <div class="rounded-2xl bg-white p-6 shadow-border">
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                <div>
                    <label class="mb-1.5 block text-sm font-medium text-neutral-700">Nama Provinsi *</label>
                ...
                <div>
                    ...
                    />
                    @error('id_negara') <p class="mt-1.5 text-sm text-red-600">{{ $message }}</p> @enderror
                </div>
            </div>
        </div>

        <div class="flex items-center justify-end gap-3">
            <a href="{{ route('admin.sistem.provinsi') }}" class="rounded-lg px-4 py-2.5 text-sm font-medium text-neutral-700 transition hover:bg-neutral-50 shadow-border">
                Batal
            </a>
